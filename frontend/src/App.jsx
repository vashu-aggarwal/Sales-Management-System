...
export default function App() {
  ...
  const [filters, setFilters] = useState({
    regions: [], gender: [], categories: [], tags: [], paymentMethods: [], ageRange: [0, 200], dateRange: [null, null]
  })
  ...
  const [options, setOptions] = useState({ regions: [], genders: [], categories: [], paymentMethods: [], tags: [] })
  ...
  const buildFilterParams = useCallback(() => {
    ...
    const appendArray = (key, arr) => {
      ...
      if (Array.isArray(arr)) {
        arr.forEach(v => {
          if (v !== undefined && v !== null && String(v) !== '') params.append(key, String(v))
        })
      } else if (String(arr).includes(',')) {
        String(arr).split(',').map(s => s.trim()).filter(Boolean).forEach(v => params.append(key, v))
      } else if (String(arr) !== '') {
    ...
    // date range
    if (filters.dateRange && filters.dateRange[0]) params.append('dateFrom', filters.dateRange[0])
    if (filters.dateRange && filters.dateRange[1]) params.append('dateTo', filters.dateRange[1])
  ...
  }, [searchQuery, filters, sortBy, page, pageSize, buildFilterParams])

  // load filter options (sample a larger page)
  const loadFilterOptions = useCallback(async () => {
    try {
      const res = await fetch(`${backendBase}/api/transaction?page=1&pageSize=1000`)
      if (!res.ok) return
      const json = await res.json()
      ...
      setOptions({
        regions: unique(sample.map(s => s.customer_region || s.customerRegion)),
        genders: unique(sample.map(s => s.gender)),
        categories: unique(sample.map(s => s.product_category || s.productCategory)),
        paymentMethods: unique(sample.map(s => s.payment_method)),
        tags: unique(sample.flatMap(s => (s.tags ? String(s.tags).split(',') : []))).map(t => t.trim()).filter(Boolean),
      })
    } catch (err) {
